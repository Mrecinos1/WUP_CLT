...
The previous graph show that the mean is getting closer and closer to 50 and standard deviation is decreasing which is in according with CLT theory. The standard deviation was as expected, because the sample standard deviation was 41.38 and when divided by square root 9 (the sample size) it equal approximately 13.93, which is close to the results which is approximately 14. overall, the graph is getting more symmetrical.
...
The graph is looking more symmetrical, because now we are using 16 samples at a time which gives for more chances for opinions discrepancies. And the standard deviation is decreasing as expected because we are dividing by a longer number. In this case it's the square root of 16 which equals 4.
...
The graph is looking more symmetrical, because now we are using 25 samples at a time which gives us more chances or opinions discrepancies. And the standard deviation is decreasing as expected because we are dividing by a larger number. In this case it's the square root of 25.
...
The graph is looking more symmetrical, because now we are using 36 samples at a time which gives us more chances or opinions discrepancies. And the standard deviation is decreasing as expected because we are dividing by a larger number. In this case it's the square root of 36. This graph, in addition to the previous ones, demonstrates the central limit theorem by showing less standard deviation as we increase the sample size.
